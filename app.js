...
const people = [
  ...
  {name:"郭陳瑞",car:3,room:2,kid:true},
  {name:"郭瑞芯",car:3,room:2,kid:true},
  {name:"王振華",car:4,room:7},
  {name:"MIKI",car:4,room:7},
  {name:"小徐",car:5,room:4,alias:"徐維鈴"},
  {name:"佳蓁",car:5,room:4,alias:"吳佳蓁"},
  {name:"王銘宏",car:5,room:12},
  {name:"王閨蜜",car:5,room:12},
  {name:"白婕妤",car:6,room:11},
  {name:"周紋妤",car:6,room:11},
  {name:"POTER",car:7,room:9,alias:"陳彥朋"},
  {name:"蕭宇程",car:7,room:9},
  {name:"張友維",car:7,room:6},
  {name:"羅曼芸",car:7,room:6},
  {name:"邱揆程",car:8,room:8},
  {name:"橘子🍊",car:8,room:8,alias:"林宜臻"},
...
const carLeader = {1:"陳怡靜",2:"陳意弘",3:"郭仲凱",4:"王振華",5:"小徐",6:"白婕妤",7:"POTER",8:"邱揆程",9:"吳佳臻",10:"范毓斌",11:"林宜潔",12:"林姿含"};
...
function renderSuggest(list, query) {
  ...
  box.innerHTML = list.map(person => {
    ...
    const alias = person.alias ? `（${escapeHtml(person.alias)}）` : "";
    return `<button type="button" role="option" data-person="${escapeHtml(person.name)}"><span>${highlight(person.name, query)}${alias}</span><i>第 ${person.car} 車${leader}</i></button>`;
  }).join("");
}
...
function dotList(members, self, car) {
  return `<ul class="dot-list">` + members.map(person => person.name === self.name
    ? `<li class="is-self"><div class="self-row"><span>${escapeHtml(person.name)}</span><em>你</em></div></li>`
...
function showTraveler(person) {
  ...
  $("query-result").innerHTML = `
    <div class="trip-card-head">
      <div>
        ...
        <p>第 ${car} 車 · 第 ${room} 房${person.alias ? " · " + escapeHtml(person.alias) : ""}</p>
      </div>
    ...
    <div class="tc-panel">
      ...
      <div class="hero-row"><i>遊覽車次</i><b>第 ${car} 車</b><u>全程請依此<br>車次集合</u></div>
      <div class="dash"></div>
      <p class="tc-label">同車夥伴 ${carList.length} 人</p>
      ${dotList(carList, person, car)}
    </div>
    ...
    <div class="tc-panel">
      ...
      ${dotList(roomList, person, car)}
    </div>

    <div class="trip-card-foot">
      <a class="act-map" href="${STAY.map}" target="_blank" rel="noopener noreferrer"><svg viewBox="0 0 24 24" aria-hidden="true"><use href="#i-pin"/></svg>民宿位置</a>
      <button class="act-share" type="button" id="share-trip"><svg viewBox="0 0 24 24" aria-hidden="true"><use href="#i-share"/></svg>分享</button>
    </div>`;
  ...
  $("share-trip").addEventListener("click", () => shareTrip(person));
  window.scrollTo({ top: 0, behavior: "smooth" });
}

function shareTrip(person) {
  const car = person.car, room = person.room;
  const others = list => list.filter(p => p.name !== person.name).map(p => p.name).join("、") || "無";
  const text = [
    `567 神秘小旅行 · ${person.name}`,
    `第 ${car} 車（車長 ${carLeader[car]}）`,
    `同車：${others(carMembers(car))}`,
    `第 ${room} 房 · ${roomType[room]}`,
    `室友：${others(roomMembers(room))}`
  ].join("\n");
  if (navigator.share) { navigator.share({ text }).catch(() => {}); return; }
  const done = () => {
    const button = $("share-trip");
    button.lastChild.textContent = "已複製";
    setTimeout(() => { if ($("share-trip")) $("share-trip").lastChild.textContent = "分享"; }, 1800);
  };
...
$("show-all").addEventListener("click", () => {
  $("search-hint").hidden = true;
  renderSuggest(people.slice().sort((a, b) => a.car - b.car), "");
});
